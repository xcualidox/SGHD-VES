...
class Login extends db_connect
{
    ...
    {
        $query = "SELECT CONCAT(p.`primer_nombre`,' ', p.`primer_apellido`) as fullname, l.`status` FROM personas AS p INNER JOIN login AS l ON p.cedula = l.username WHERE p.`cedula` = ? ";
        $result = $this->query($query, $this->username);
        return $result ? $this->fetch_query($result) : false;
    }
