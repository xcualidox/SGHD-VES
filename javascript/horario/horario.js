...
function DeshabilitarAulaGrupo() {
  ...
  for (let i = 0; i < aulasAhabilitadas.length; i++) {
    if(aulasAhabilitadas[i].value==aulaB.value && aulasAhabilitadas[i].value!=''){
      aulasAhabilitadas[i].disabled=true;
    }
    else{
      aulasAhabilitadas[i].disabled=false;
    }
  }

  for (let i = 0; i < aulasBhabilitadas.length; i++) {
    if(aulasBhabilitadas[i].value==aulaA.value && aulasBhabilitadas[i].value!=''){
      aulasBhabilitadas[i].disabled=true;
    }
    else{
      aulasBhabilitadas[i].disabled=false;
    }
...
function EditarBloque(bloque) {
  ...
  console.log("Seccion Horario Docente ",anoSeccion);
    ...
    let aulainput=document.getElementById('aula');
    let materiainput=document.getElementById('materia');
    let profesorinput=cedulaDocente;
    ...
    //Si no los hay, vaciarlos
      ...
      aulainput.value = '';
      materiainput.value = '';
      profesorinput.value = cedulaDocente;
      aula2input.value = '';
      materia2input.value = '';
...
function GuardarHorario() {
     ...
     document.getElementById('CedulaDocenteModificar').value=cedulaDocente;
